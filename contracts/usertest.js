...
const ECHO_VOTER_ADDRESS = "0xYourEchoVoterAddress";
...
describe("Vote Contract Test (Using Deployed Contracts)", function () {
    ...
    before(async function () {
        ...
        echoVoter = await ethers.getContractAt("EchoVoter", ECHO_VOTER_ADDRESS);
    });

    it("Alice 选择 AI 代理投票（Reject），Bob 和 David 亲自投票（Approve）", async function () {
        // Alice 使用 AI 代理投票（Reject: 0）
        await echoVoter.connect(alice).storeAIVote(0);
