...
async function handleAIAgentApproval(approved, option) {
    ...
    try {
        ...
        const echoVoter = new ethers.Contract(ECHO_VOTER_ADDRESS, EchoVoterABI, signer);
        
        // 调用合约函数存储 AI 代理的投票
        const tx = await echoVoter.storeAIVote(option);
        await tx.wait();
        console.log(`AI 代理投票成功: 选项 ${option}`);

        // 监听最低投票要求是否达成（避免重复监听）
        echoVoter.once("QuorumReached", async (totalVotes) => {
